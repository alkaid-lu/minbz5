...
class VariationalAutoEncoder(nn.Module):
    ...
    def __decode_latent(self, latent, stdMult = 1.0):
        mean, var = torch.chunk(latent, 2, dim=-1)
        std = var.mul(0.5).exp_()
        esp = torch.randn(*mean.size())*stdMult
    ...
    def forward(self, feature):
        ...
        if self.training:
            ...
            for i in range(10):
                stdMult = 0.5*(2**i)#从0.5开始，每次乘以2 
                reconstruct, properties = self.__decode_latent(latent, stdMult)
